...
%data collection
S_save=ones(n,length(r_s));
A_save=ones(n,n,length(r_s));
pdif=ones(N,time/down,length(r_s),length(k_s));
adif=ones(N,time/down,length(r_s),length(k_s));
energy_func=ones(N,time/down,length(r_s),length(k_s));
energy_delta=ones(N,time/down,length(r_s),length(k_s));
energy_pair=ones(N,time/down,length(r_s),length(k_s));
sum_friendly=zeros(length(r_s),time/down,length(k_s));
Bal_tri_sum=zeros(length(r_s),time/down,length(r_s));
%% Setting
for p=1:length(r_s)
    ...
    for p2=1:length(k_s)
        fprintf('complete: %.1f \n', (p2+(p-1)*(length(k_s)))/(length(k_s)*length(r_s))*100)
        k=k_s(p2)*delta_t;
        b=(b0-k_s(p2))*delta_t; %infection rate
        ba=b*a0;
        d=d0*delta_t;
        for o=1:N
            A=A_init;
...
marker=["--*","-o",":d","-.x","--^","-.",":+","-.o","--d","-x",":^","-.."];
for i=1:length(k_s)
    plot(r_s,p_av(end,:,i),marker(i),'DisplayName',['b=',num2str(b0-k_s(i)),' k=',num2str(k_s(i))])
end
xlabel('r0')
...
marker=["--*","-o",":d","-.x","--^","-.",":+","-.o","--d","-x",":^","-.."];
for i=1:length(k_s)
    plot(r_s,energy_av(end,:,i),marker(i),'DisplayName',['b=',num2str(b0-k_s(i)),' k=',num2str(k_s(i))])
end
xlabel('r0')
ylabel('Energy delta')
legend
