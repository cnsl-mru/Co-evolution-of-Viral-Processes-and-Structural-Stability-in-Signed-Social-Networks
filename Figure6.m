...
d0=9; %recovery rate
dif_alpha=0:0.1:1; %coef. for energy function
delta_t=0.01;
...
%data collection
S_save=ones(n,length(dif_alpha));
A_save=ones(n,n,length(dif_alpha));
pdif=ones(N,time/down,length(dif_alpha),length(r_s));
adif=ones(N,time/down,length(dif_alpha),length(r_s));
energy_func=ones(N,time/down,length(dif_alpha),length(r_s));
energy_delta=ones(N,time/down,length(dif_alpha),length(r_s));
energy_pair=ones(N,time/down,length(dif_alpha),length(r_s));
sum_friendly=zeros(length(dif_alpha),time/down,length(r_s));
Bal_tri_sum=zeros(length(dif_alpha),time/down,length(r_s));
%% Setting
for p2=1:length(r_s)
    ...
    for p=1:length(dif_alpha)
        fprintf('complete: %.1f \n', (p+(p2-1)*(length(dif_alpha)))/(length(r_s)*length(dif_alpha))*100)
        alpha=dif_alpha(p); %alerting rate
        k=k_s*delta_t;
...
%% Infection density vs alpha
figure
p_av=squeeze(sum(pdif)/N);
t=linspace(1,actual_time,length(p_new));
hold on
marker=["--*","-o",":d","-.x","--^","-.",":+","-.o","--d","-x",":^","-.."];
for i=1:length(r_s)
    plot(dif_alpha,p_av(end,:,i),marker(i),'DisplayName',['r0=',num2str(r_s(i))])
end
xlabel('alpha')
...
%% Infection density vs alpha
figure
energy_av=squeeze(sum(energy_delta)/N);
hold on
marker=["--*","-o",":d","-.x","--^","-.",":+","-.o","--d","-x",":^","-.."];
for i=1:length(r_s)
    plot(dif_alpha,energy_av(end,:,i),marker(i),'DisplayName',['r0=',num2str(r_s(i))])
end
xlabel('alpha')
ylabel('Energy delta')
legend
...
%% Infection density vs alpha
figure
energy_av=squeeze(sum(energy_pair)/N);
hold on
marker=["--*","-o",":d","-.x","--^","-.",":+","-.o","--d","-x",":^","-.."];
for i=1:length(r_s)
    plot(dif_alpha,energy_av(end,:,i),marker(i),'DisplayName',['r0=',num2str(r_s(i))])
...
%% Energy
figure
energy_av=squeeze(sum(energy_func)/N);
hold on
for i=1:length(r_s)
    plot(dif_alpha,energy_av(end,:,i),marker(i),'DisplayName',['r0=',num2str(r_s(i))])
end
...
surf(r_s,dif_alpha,squeeze(p_av(:,:,end)))
ylabel('alpha')
xlabel('r0')
...
figure
if N==1
    energy_func_p=squeeze(energy_func);
else
    energy_func_p=sum(energy_func)/N;
    energy_func_p=squeeze(energy_func_p);
end
energy_func_p=permute(energy_func_p, [2 3 1]);

surf(r_s,dif_alpha,squeeze(energy_func_p(:,:,end)))
ylabel('alpha')
xlabel('r0')
zlabel('Network energy')
